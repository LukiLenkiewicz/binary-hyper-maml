    ...
    for epoch in range(start_epoch, stop_epoch):
        ...
        if (epoch % params.eval_freq == 0) or epoch in [
            ...
            if acc > max_acc:  # for baseline and baseline++, we don't use validation here so we let acc = -1
                ...
                if params.maml_save_feature_network and params.method in ['maml', 'hyper_maml','bayes_hmaml', 'binary_maml']:
...
if __name__ == '__main__':
    ...
    if params.dataset == 'cross' and params.method == "vbh_maml":
        base_file = configs.data_dir['miniImagenet'] + 'all_vit.json'
        val_file = configs.data_dir['CUB'] + 'val_vit.json'
    elif params.dataset == 'cross_char' and params.method == "vbh_maml":
        base_file = configs.data_dir['omniglot'] + 'noLatin_vit.json'
        val_file = configs.data_dir['emnist'] + 'val_vit.json'
    ...
    base_datamgr = EmbDataManager(**train_few_shot_params)  # n_eposide=100
    ...
    val_datamgr = EmbDataManager(**test_few_shot_params)
    val_loader = val_datamgr.get_data_loader(val_file)

    model = VBHMAML(params=params, approx=(params.method == 'maml_approx'),
